...
-- Insert sample posts
INSERT INTO posts (title, slug, content, author_id, category_id, created_at, view_count, like_count, reply_count)
SELECT 
  CASE 
  ...
  CASE 
    WHEN c.name = 'Space & Astronomy' THEN 
      ...
      END
    WHEN c.name = 'Technology' THEN 'ai-and-machine-learning-in-2024'
    WHEN c.name = 'Gaming' THEN 'best-indie-games-you-havent-played-yet'
    WHEN c.name = 'Science Fiction' THEN 'dune-part-two-discussion-thread'
    ELSE 'welcome-to-interstellar-nerd-forum'
  END as slug,
  CASE 
...
SELECT 
  ...
  FLOOR(RANDOM() * 8 + 1)
FROM (VALUES 
  ('SpaceX Starship Latest Test Flight', 'spacex-starship-latest-test-flight', 'Did anyone else watch the latest Starship test? The progress they''re making is incredible. The landing attempt was so close! What do you think about their rapid iteration approach vs traditional aerospace development?'),
  ('Exoplanet Discovery: Earth-like Planet Found', 'exoplanet-discovery-earth-like-planet-found', 'Astronomers just discovered an Earth-like planet in the habitable zone of a nearby star system. The planet appears to have an atmosphere and possibly liquid water. This could be one of the most promising candidates for potential life outside our solar system.'),
  ('Black Hole Photography Breakthrough', 'black-hole-photography-breakthrough', 'The Event Horizon Telescope team has released new images of black holes with unprecedented detail. The ring of light around the event horizon is absolutely mesmerizing. How do they even capture images of something that doesn''t emit light?')
) AS sample_posts(title, slug, content)
